"""Quality push: find best compression config at <1% PPL on Mistral-7B.

Strategies tested:
  A: 3-bit + eviction sweep (0%, 40%-75%)
  B: 2-bit fine grid near the cliff (32%, 35%, 38%)
  C: Mixed bits k2v3 + eviction sweep (0%, 40%, 50%, 60%)

Both EASY and HARD text corpora from the previous sweep.
"""
import modal
import os

app = modal.App("nexusquant-quality-push")
...
def run_quality_push():
    ...
    print("=" * 80)
    print("NEXUSQUANT — Quality Push (Mistral-7B)")
    print("Strategies A (3b+evict), B (2b fine grid), C (k2v3 mixed)")
    print("=" * 80)
    ...
    def evict_quantize(kv_cache, keep_mask, key_bits, val_bits, prefix_len):
        ...
        key_bits / val_bits allow asymmetric quantization (k2v3).
        Compression accounting includes:
          - zstd-22 compressed indices (delta-coded per token)
          - fp16 scale per vector (one scale per kept token per head per layer)
    ...
    def run_configs_on_text(text_name, text, configs):
        ...
        configs: list of dicts with keys:
          name, evict_pct, key_bits, val_bits
        """
    ...
    # Strategy A: 3-bit + eviction sweep
    for ep in [0, 40, 50, 55, 60, 65, 70, 75]:
        label = f"A 3b+{ep}%evict" if ep > 0 else "A 3b no-evict"
        configs.append({"name": label, "evict_pct": ep, "key_bits": 3, "val_bits": 3})

    # Strategy B: 2-bit fine grid near the cliff
    for ep in [32, 35, 38]:
        configs.append({"name": f"B 2b+{ep}%evict", "evict_pct": ep, "key_bits": 2, "val_bits": 2})

    # Strategy C: Mixed bits k2v3 + eviction
    for ep in [0, 40, 50, 60]:
        label = f"C k2v3+{ep}%evict" if ep > 0 else "C k2v3 no-evict"
        configs.append({"name": label, "evict_pct": ep, "key_bits": 2, "val_bits": 3})
    ...
    # ------------------------------------------------------------------ summary table
    print(f"\n{'='*80}")
    print("QUALITY PUSH SUMMARY TABLE")
    print(f"{'='*80}")
    print(f"\n{'Config':<28s} {'Easy Δ%':>8s} {'Hard Δ%':>8s} {'Max Δ%':>8s} {'Ratio':>7s} {'<1%?':>6s}")
    print("-" * 72)
    ...
    for cfg in configs:
        ...
        if easy and hard:
            ...
            print(f"{name:<28s} {easy['delta']:+7.2f}% {hard['delta']:+7.2f}% "
    ...
    if best_config:
        ...
        print(">>> No config achieved <1% on BOTH texts.")
        # show best near-miss
        combos = []
        for cfg in configs:
            name = cfg["name"]
...
def main():
    import time
    print("Launching quality push experiment on Modal A10G...")
    all_results, best_config = run_quality_push.remote()

    out_dir = os.path.join(os.path.dirname(__file__), "..", ".company", "engineering")
    os.makedirs(out_dir, exist_ok=True)
    out = os.path.join(out_dir, "quality_push_results.md")

    with open(out, "w") as f:
        f.write("# Quality Push Results — Mistral-7B\n\n")
        f.write(f"**Date:** {time.strftime('%Y-%m-%d %H:%M')}\n")
        f.write("**Goal:** Find best compression config at <1% PPL on BOTH easy and hard text\n\n")
        f.write("## Strategy Key\n")
        f.write("- A: 3-bit E8 quantization + eviction sweep\n")
        f.write("- B: 2-bit E8 quantization, fine-grid eviction near quality cliff\n")
        f.write("- C: Mixed bits (keys=2-bit, values=3-bit) + eviction\n\n")
        ...
        for cfg in [{"name": c["name"]} for c in (
            [{"name": f"A 3b+{ep}%evict" if ep > 0 else "A 3b no-evict"} for ep in [0,40,50,55,60,65,70,75]] +
            [{"name": f"B 2b+{ep}%evict"} for ep in [32,35,38]] +
            [{"name": f"C k2v3+{ep}%evict" if ep > 0 else "C k2v3 no-evict"} for ep in [0,40,50,60]]
        )]:
            nm = cfg["name"]
            if nm in seen and "easy" in seen[nm] and "hard" in seen[nm]:
                e = seen[nm]["easy"]
                h = seen[nm]["hard"]
